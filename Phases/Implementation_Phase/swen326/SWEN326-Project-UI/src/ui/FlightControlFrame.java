...
public class FlightControlFrame extends JFrame {
	...
	public FlightControlFrame() {
		...
		this.map = new ImageIcon(new ImageIcon("src/ui/Images/map.png").getImage().getScaledInstance(this.mapWidth,
				this.mapHeight, Image.SCALE_DEFAULT));
		this.autopilotOff = new ImageIcon(
				new ImageIcon("src/ui/Images/off_light.png").getImage().getScaledInstance(75, 75, Image.SCALE_DEFAULT));
		this.autopilotOn = new ImageIcon(new ImageIcon("src/ui/Images/green_light.png").getImage().getScaledInstance(75,
				75, Image.SCALE_DEFAULT));
		this.autopilotFaulty = new ImageIcon(
				new ImageIcon("src/ui/Images/red_light.png").getImage().getScaledInstance(75, 75, Image.SCALE_DEFAULT));
	 ...
	 * @param time
	 */
	private void addWayPoint(String name, double latitude, double longitude, double altitude, double speed,
			String time) {
	...
	protected void drawMap(Graphics g) {
		...
		for (WayPoint wp : waypoints) {
			...
			// To plot the route connecting the input waypoints
			if (paintRoute) {
				if (i >= 1) {
					Graphics2D g2d = (Graphics2D) g;
					g2d.setStroke(new BasicStroke(2));
					g2d.drawLine(x + 5 / 2, y + 5 / 2, waypoints.get(i - 1).coordinate().getX(mapWidth),
							waypoints.get(i - 1).coordinate().getY(mapHeight));
				}
			}
			i++;
		}
	...
	private JPanel getAutopilotControlPanel(Border border) {
		...
		autopilotControlPanel.setBounds((int) ((11.0 / 14.0) * width), 0, (int) ((3.0 / 14.0) * width),
				(int) ((3.0 / 4.0) * height) - 35);
		autopilotControlPanel.setBackground(Color.DARK_GRAY);
		autopilotControlPanel.setBorder(border);
		autopilotControlPanel.setLayout(null);

		// Components
		autopilotEngagedButton = new JCheckBox("Autopilot Engaged");
		autopilotEngagedButton.setBounds(20, 25, 150, 30);
		...
		autopilotLight.setBounds(160, 0, 100, 100);

		this.modifyLabel(altitudeLabel);
		this.modifyLabel(thrustLabel);

		altitudeSlider = new JSlider(1, 0, 30000, 0);
		altitudeSlider.addChangeListener(e -> {
			altitudeValue = altitudeSlider.getValue();
			altitudeLabel.setText("Set Altitude : " + ((JSlider) e.getSource()).getValue());
		});
		...
		autopilotControlPanel.add(thrustSlider);
		autopilotControlPanel.add(altitudeLabel);
		autopilotControlPanel.add(thrustLabel);

		return autopilotControlPanel;

	}
	/**
	 * Get the Altitude Value in Manual Override mode
	 * @return altitude
	 */
	...
	/**
	 * Get the thrust value in Manual Override mode
	 * @return thrust
	 */
